...
const HotGameSlideshow = ({ games, name, title, length, onGameClick, loadMoreContent }) => {
    ...
    return (
        <section className="livecasino-lobby col-12 order-4">
            <div className="section team-section wow fadeIn d-flex align-items-center justify-content-between mt-5" data-wow-delay="0.3s">
                <h3 className="text-left mt-0 mb-0 h1 category-title">{title}</h3>
                {
                    loadMoreContent && <div className="load-more" onClick={loadMoreContent}>Ver todos</div>
                }
            </div>
            <div className="swiper-container swiper-livecasino swiper-initialized swiper-horizontal">
                <div
                    className={`swiper-button-prev ${isPrevDisabled ? "d-none" : ""}`}
                    onClick={handlePrev}
                    role="button"
                    tabIndex={0}
                ...
                <div
                    className={`swiper-button-next ${isNextDisabled ? "d-none" : ""}`}
                    onClick={handleNext}
                    role="button"
                    tabIndex={0}
                ...
                <Swiper
                    ...
                    breakpoints={{
                        ...
                        1200: { slidesPerView: length ? 7 : 5, spaceBetween: 40 },
                    }}
                    navigation={{
